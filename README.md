...
Press **Ctrl+Shift+E** to open tab-oil in a regular browser tab (configurable in `chrome://extensions/shortcuts` — e.g. change it to `Ctrl+Minus`). This avoids creating a separate desktop window, which is important for tiling window managers.
...
**Note:** Window, folder, saved-section, and divider rows are **non-editable**. Tab rows remain fully editable.
...
- Browser tab groups are deliberately not shown in the editable buffer and are never changed by `:w`.
